...
SCENARIOS = {
    ...
    # CALIBRATION NOTES
    # -----------------
    # SERVICE_RATE = 0.15 veh/s (~6.7s per vehicle) — realistic saturation flow
    # A 30s green serves ~4-5 vehicles, so queues build to 20-50 cars under load.
    #
    # MAX_QUEUE_THRESHOLD is set per-scenario to reflect physical lane capacity:
    #   Low/medium scenarios  → threshold 20  (queues 5-15,  rarely blocked)
    #   High load scenarios   → threshold 15  (queues 15-30, occasional blocks)
    #   Near-sat scenarios    → threshold 10  (queues 25-50, frequent blocks)
    #   Overloaded scenarios  → threshold 10  (queues grow unbounded, many blocks)
    #
    # ρ = λ / μ.  With μ=0.15:
    #   λ=0.05  → ρ=0.33  (light,          ~2-5  cars/lane)
    #   λ=0.09  → ρ=0.60  (moderate,       ~8-15 cars/lane)
    #   λ=0.12  → ρ=0.80  (busy,           ~18-28 cars/lane)
    #   λ=0.135 → ρ=0.90  (heavy,          ~25-40 cars/lane)
    #   λ=0.143 → ρ=0.95  (near-sat,       ~30-50 cars/lane)
    #   λ=0.16  → ρ=1.07  (unstable,       queues → ∞)
    #
    # SIMULATION_DURATION = 3600s, WARMUP_PERIOD = 300s throughout
    # =========================================================================

    # -----------------------------------------------------------------------
    # GROUP 1: Traffic Load Scenarios  (2x2 grid, symmetric)
    # Threshold drops as load rises to ensure blockages appear at high load
    # -----------------------------------------------------------------------
    "1A_low_traffic": {
        "_description": "Best Case - Low Traffic (rho=0.33, ~2-5 cars/lane, no blocks)",
        "ARRIVAL_RATE": 0.05,
        "SERVICE_RATE": 0.15,
    ...
    "1B_medium_traffic": {
        "_description": "Medium Case - Moderate Traffic (rho=0.60, ~8-15 cars/lane, rare blocks)",
        "ARRIVAL_RATE": 0.09,
        "SERVICE_RATE": 0.15,
        "USE_ASYMMETRIC_TRAFFIC": False,
        "ERLANG_K": 2,
    ...
    "1C_high_traffic": {
        "_description": "High Traffic (rho=0.80, ~18-28 cars/lane, occasional blocks)",
        "ARRIVAL_RATE": 0.12,
        "SERVICE_RATE": 0.15,
        "USE_ASYMMETRIC_TRAFFIC": False,
        "ERLANG_K": 2,
    ...
    "1D_near_saturation": {
        "_description": "Near Saturation (rho=0.95, ~30-50 cars/lane, frequent blocks)",
        "ARRIVAL_RATE": 0.143,
        "SERVICE_RATE": 0.15,
        "USE_ASYMMETRIC_TRAFFIC": False,
        "ERLANG_K": 2,
    ...
    "1E_overloaded": {
        "_description": "Overloaded / Unstable (rho=1.07, queues unbounded, gridlock)",
        "ARRIVAL_RATE": 0.16,
        "SERVICE_RATE": 0.15,
        "USE_ASYMMETRIC_TRAFFIC": False,
        "ERLANG_K": 2,
    ...
    # Low pair at rho=0.60 (rarely blocked), High pair at rho=0.80 (some blocks)
    # M/M/1 has higher variance -> more blockages than M/G/1 at same load
    # -----------------------------------------------------------------------
    "2A_MM1_low": {
        "_description": "M/M/1 - Moderate Traffic (rho=0.60, higher variance)",
        "ERLANG_K": 1,
        "ARRIVAL_RATE": 0.09,
        "SERVICE_RATE": 0.15,
        "USE_ASYMMETRIC_TRAFFIC": False,
    ...
    "2B_MG1_low": {
        "_description": "M/G/1 Erlang-2 - Moderate Traffic (rho=0.60, lower variance)",
        "ERLANG_K": 2,
        "ARRIVAL_RATE": 0.09,
        "SERVICE_RATE": 0.15,
        "USE_ASYMMETRIC_TRAFFIC": False,
    ...
    "2C_MM1_high": {
        "_description": "M/M/1 - High Traffic (rho=0.80, expect more blocks than 2D)",
        "ERLANG_K": 1,
        "ARRIVAL_RATE": 0.12,
        "SERVICE_RATE": 0.15,
        "USE_ASYMMETRIC_TRAFFIC": False,
    ...
    "2D_MG1_high": {
        "_description": "M/G/1 Erlang-2 - High Traffic (rho=0.80, fewer blocks than 2C)",
        "ERLANG_K": 2,
        "ARRIVAL_RATE": 0.12,
        "SERVICE_RATE": 0.15,
        "USE_ASYMMETRIC_TRAFFIC": False,
    ...
    "2E_MG1_high_regular": {
        "_description": "M/G/1 Erlang-5 - High Traffic, Very Regular (rho=0.80, fewest blocks)",
        "ERLANG_K": 5,
        "ARRIVAL_RATE": 0.12,
        "SERVICE_RATE": 0.15,
        "USE_ASYMMETRIC_TRAFFIC": False,
    ...
    # Asymmetric scenarios use heavier loads on some lanes -> more blockages
    # -----------------------------------------------------------------------
    "3A_symmetric_low": {
        "_description": "Symmetric Traffic - Low Load (rho=0.60, uniform, rarely blocked)",
        "ARRIVAL_RATE": 0.09,
        "SERVICE_RATE": 0.15,
        "USE_ASYMMETRIC_TRAFFIC": False,
        "ERLANG_K": 2,
    ...
    "3B_symmetric_high": {
        "_description": "Symmetric Traffic - High Load (rho=0.80, uniform, occasional blocks)",
        "ARRIVAL_RATE": 0.12,
        "SERVICE_RATE": 0.15,
        "USE_ASYMMETRIC_TRAFFIC": False,
        "ERLANG_K": 2,
    ...
    "3C_asymmetric_moderate": {
        "_description": "Asymmetric Traffic - Moderate (rho 0.73-0.87, hot lanes occasionally block)",
        "USE_ASYMMETRIC_TRAFFIC": True,
        "LANE_ARRIVAL_RATES": {
            0: {"N": 0.11, "S": 0.13, "E": 0.13, "W": 0.11},
            1: {"N": 0.13, "S": 0.11, "E": 0.11, "W": 0.13},
            2: {"N": 0.13, "S": 0.11, "E": 0.11, "W": 0.13},
            3: {"N": 0.11, "S": 0.13, "E": 0.13, "W": 0.11},
        },
        "ARRIVAL_RATE": 0.12,
        "SERVICE_RATE": 0.15,
        "ERLANG_K": 2,
    ...
    "3D_asymmetric_rush_hour": {
        "_description": "Rush Hour - Heavy directional bias (hot lanes rho=0.87, cold rho=0.33, many blocks)",
        "USE_ASYMMETRIC_TRAFFIC": True,
        "LANE_ARRIVAL_RATES": {
            0: {"N": 0.05, "S": 0.13, "E": 0.13, "W": 0.05},
            1: {"N": 0.13, "S": 0.05, "E": 0.05, "W": 0.13},
            2: {"N": 0.05, "S": 0.13, "E": 0.13, "W": 0.05},
            3: {"N": 0.13, "S": 0.05, "E": 0.05, "W": 0.13},
        },
        "ARRIVAL_RATE": 0.12,
        "SERVICE_RATE": 0.15,
        "ERLANG_K": 2,
    ...
    # Larger grids concentrate traffic -> more blockages at same arrival rate
    # -----------------------------------------------------------------------
    "4A_linear_3": {
        "_description": "Linear Corridor - 3 Intersections (rho=0.60, rarely blocked)",
        "NUM_INTERSECTIONS": 3,
    ...
    "4B_grid_2x2": {
        "_description": "2x2 Grid - 4 Intersections (rho=0.80, occasional blocks)",
        "NUM_INTERSECTIONS": 4,
        "NETWORK_TOPOLOGY": TOPOLOGY_2x2,
        "ARRIVAL_RATE": 0.12,
        "SERVICE_RATE": 0.15,
    ...
    "4C_t_junction": {
        "_description": "T-Junction - Center heavily loaded (rho=0.80), outer light (rho=0.47)",
        "NUM_INTERSECTIONS": 3,
        "NETWORK_TOPOLOGY": TOPOLOGY_T_3,
        "USE_ASYMMETRIC_TRAFFIC": True,
        "LANE_ARRIVAL_RATES": {
            0: {"N": 0.07, "S": 0.07, "E": 0.07, "W": 0.07},
            1: {"N": 0.12, "S": 0.12, "E": 0.12, "W": 0.12},
            2: {"N": 0.07, "S": 0.07, "E": 0.07, "W": 0.07},
        },
        "ARRIVAL_RATE": 0.09,
        "SERVICE_RATE": 0.15,
        "ERLANG_K": 2,
    ...
    "4D_grid_3x3": {
        "_description": "3x3 Grid - 9 Intersections (rho=0.80, 18 variables, blocks at center)",
        "NUM_INTERSECTIONS": 9,
    ...
    # -----------------------------------------------------------------------
    # GROUP 5: PSO vs ACO Algorithm Comparison
    # Low = rarely blocked, High/Near-sat = clearly blocked -> shows value of opt
    # -----------------------------------------------------------------------
    "5A_algo_low_load": {
        "_description": "Algorithm Comparison - Low Load (rho=0.60, rarely blocked)",
        "ARRIVAL_RATE": 0.09,
        "SERVICE_RATE": 0.15,
        "USE_ASYMMETRIC_TRAFFIC": False,
        "ERLANG_K": 2,
    ...
    "5B_algo_high_load": {
        "_description": "Algorithm Comparison - High Load Asymmetric (rho~0.80, blocks on hot lanes)",
        "ARRIVAL_RATE": 0.12,
        "USE_ASYMMETRIC_TRAFFIC": True,
        "LANE_ARRIVAL_RATES": {
            0: {"N": 0.11, "S": 0.13, "E": 0.13, "W": 0.11},
    ...
    "5C_algo_near_saturation": {
        "_description": "Algorithm Comparison - Near Saturation (rho=0.95, frequent blocks)",
        "ARRIVAL_RATE": 0.143,
        "SERVICE_RATE": 0.15,
        "USE_ASYMMETRIC_TRAFFIC": False,
        "ERLANG_K": 2,
    ...
    # -----------------------------------------------------------------------
    # GROUP 6: Signal Timing Bounds  (all at rho=0.80, threshold=15)
    # Tight bounds -> optimizer can't help much -> more blocks remain
    # Wide bounds  -> optimizer has freedom -> fewer blocks
    # -----------------------------------------------------------------------
    "6A_tight_bounds": {
        "_description": "Tight Bounds [15,60] - High Load (optimizer constrained, more blocks)",
        "MIN_GREEN_TIME": 15,
        "MAX_GREEN_TIME": 60,
        "ARRIVAL_RATE": 0.12,
        "SERVICE_RATE": 0.15,
    ...
    "6B_standard_bounds": {
        "_description": "Standard Bounds [20,90] - High Load (baseline bounds)",
        "MIN_GREEN_TIME": 20,
        "MAX_GREEN_TIME": 90,
        "ARRIVAL_RATE": 0.12,
        "SERVICE_RATE": 0.15,
    ...
    "6C_wide_bounds": {
        "_description": "Wide Bounds [10,120] - High Load (most freedom, fewest blocks)",
        "MIN_GREEN_TIME": 10,
        "MAX_GREEN_TIME": 120,
        "ARRIVAL_RATE": 0.12,
        "SERVICE_RATE": 0.15,
    ...
    # -----------------------------------------------------------------------
    # GROUP 7: Hyperparameter Sensitivity  (rho=0.80, threshold=15)
    # All at high load so differences between w/q values are visible
    # -----------------------------------------------------------------------
    "7A_pso_w_low": {
